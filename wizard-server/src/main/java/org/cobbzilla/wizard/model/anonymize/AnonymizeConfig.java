...
public class AnonymizeConfig {
    ...
    private static AnonTable createAnonTable(Class clazz) {
        ...
        List<AnonColumn> anonColumns = new ArrayList<>();
        AnonymizeList anonymizeDefaultList = (AnonymizeList) clazz.getAnnotation(AnonymizeList.class);
        if(anonymizeDefaultList != null){
            for(String name: anonymizeDefaultList.list()){
        ...
        for (Field f : fields) {
            Annotation a = f.getAnnotation(AnonymizeType.class);
            if (a != null) {
                AnonymizeType anonymizeType = (AnonymizeType)a;
                anonColumns.add(createAnonColumn(StringUtil.camelCaseToSnakeCase(f.getName()), anonymizeType));
            }
        }
        Method allMethods[] = clazz.getDeclaredMethods();
        for (Method m : allMethods){
            Annotation a = m.getAnnotation(AnonymizeType.class);
            if (a != null) {
                AnonymizeType anonymizeType = (AnonymizeType)a;
    ...
    private static List<Class> findClasses(String packageName) throws IOException, ClassNotFoundException {
        ...
        for (Resource resource : resources) {
            if (resource.isReadable()) {
                ...
                if (c.getAnnotation(AnonymizeList.class) != null || checkFieldsAndMethods(c)){candidates.add(c);}
            }
        }
        return candidates;
    }
